...
RegisterServerEvent('CodeRouge')
AddEventHandler('CodeRouge', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local xPlayers	= ESX.GetPlayers()
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'LSPD', '~b~Defcons', 'La ~b~LSPD~s~ passe en code ~r~rouge~s~ !\nNe vous plaignez pas des fouilles.', 'CHAR_CALL911', 8)
	end
end)

RegisterServerEvent('CodeOrange')
AddEventHandler('CodeOrange', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local xPlayers	= ESX.GetPlayers()
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
				TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'LSPD', '~b~Defcons', 'La ~b~LSPD~s~ passe en code ~o~orange~s~ !\nNe vous plaignez pas de la violence', 'CHAR_CALL911', 8)
	end
end)

RegisterServerEvent('CodeVert')
AddEventHandler('CodeVert', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local xPlayers	= ESX.GetPlayers()
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
				TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'LSPD', '~b~Defcons', 'La ~b~LSPD~s~ passe en code ~g~vert~s~ !\nRetour à la normal.', 'CHAR_CALL911', 8)
	end
end)
